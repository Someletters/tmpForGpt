...
contract LaunchBridge is UUPSUpgradeable, Ownable2StepUpgradeable, PausableUpgradeable {
    ...
    // Address to transition to.
    Proposal public proposedMainnetBridge;
    ...
    function _authorizeUpgrade(address target) internal override onlyOwner {
        ...
        require(target.codehash == proposedCodehash, "Not proposed upgrade codehash");
    ...
    function proposeTransition(address proposedBridge) external onlyOwner {
        proposedBridgeReadyAt = block.timestamp + 1 days;
        proposedMainnetBridge = Proposal({
          to: proposedBridge,
          codehash: proposedBridge.codehash
        });

        emit ProposeTransition(proposedBridge);
    }

    function cancelProposeTransition() external onlyOwner {
        require(proposedBridgeReadyAt > 0, "No bridge scheduled");

        _resetProposedBridge();
    }

    function _resetProposedBridge() internal {
        proposedBridgeReadyAt = 0;
        proposedMainnetBridge = Proposal(address(0), bytes32(0));

        emit ResetProposedTransition();
    }
    ...
    function enableTransition(address mainnetBridge) external onlyOwner {
        ...
        address proposedAddress = proposedMainnetBridge.to;
        bytes32 proposedCodehash = proposedMainnetBridge.codehash;
        require(proposedAddress != address(0x00), "No proposed bridge");
        require(block.timestamp > proposedBridgeReadyAt, "Bridge not ready");

        require(mainnetBridge == proposedAddress, "Not proposed bridge address");
        require(mainnetBridge.codehash == proposedCodehash, "Not proposed bridge codehash");
    ...
    function transition() external {
        ...
        if (daiAmountToMove > 0) {
            mainnetBridge.depositERC20To(address(DAI), address(DAI), user, daiAmountToMove, 0, bytes(""));
        }
    }

    /// Allows a user to reclaim their ETH / stETH / DAI when a proposal or upgrade
    /// is live. Reverts otherwise.
    function withdrawAndLosePoints() external {
        require(proposedBridgeReadyAt > 0 || proposedUpgradeReadyAt > 0, "No upgrade scheduled");
        _withdraw();
    }
